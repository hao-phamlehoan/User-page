...
function Body() {
  ...
  const handleSaveInfor = async () => {
    try {
      ...
      await callApi.putInformation(newUser);
      await localStorage.setItem('user', JSON.stringify(newUser1));
      window.alert("Save successed !!!")
      for (let i = 0; i < 2; i++) {
              ...
              <div className="information">
                ...
                <ul className="information-list">
                  ...
                  <li className="information-item">
                    <fieldset>
                      <legend>Email</legend>
                      <p className={changeInfor ? "input" : "input"}>{emailI}</p>
                    </fieldset>
                  </li>
                </ul>
